...
- タスク CRUD（作成・完了・未完了に戻す・やらない・削除）
- タスクカードをクリックで詳細シートを開き、シート内でタイトル・メモ・カテゴリ・予定日を直接編集（800ms デバウンス自動保存）
- 詳細シートはスマホ: Drawer（bottom）/ PC: Dialog で表示
- タスク追加モーダルのカテゴリ選択を折り返し（flex-wrap）表示に変更（横スクロール廃止）
- タスク追加モーダルのサイズ（sm:max-w-lg）・レイアウト（ヘッダー/スクロール本文/フッター分離）を統一
- お気に入り（★）トグル機能（タスクカード常時表示、詳細シートでも操作可能）
...
- CategoryFilter にフィルタ開閉ボタン（PC・モバイル共通）。カテゴリチップは折り返し表示（flex-wrap）
- フィルターパネル（FilterPanel）を開いた状態でスクロール時に画面上部固定（CategoryFilter + FilterPanel をまとめて sticky）。背景色 bg-muted
- ドラッグ＆ドロップ並び替え（displayOrder Float 方式、フィルタなし時のみ有効）
- カテゴリ表示順並び替え（sortOrder Int、/categories ページで D&D）
- カテゴリ説明文（description）フィールド追加（nullable、最大200文字）。編集ダイアログで入力・一覧でサブテキスト表示
...
- CategoryFilter のトグルボタン（SlidersHorizontal）は `items-start` で常に上端揃え（カテゴリが2行以上になっても自然に見える）
